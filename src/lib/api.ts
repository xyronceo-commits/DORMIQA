...
export async function createListing(listingData: Partial<Listing>): Promise<Listing> {
  ...
  try {
    ...
    return {
      ...
      agentId: listingData.agentId || 'agent_001',
      agentName: listingData.agentName || 'Agent',
      agentPhone: listingData.agentPhone || '+234 800 000 0000',
      agentEmail: listingData.agentEmail || 'agent@dormiqa.africa',
...
export async function bookInspection(bookingData: Partial<InspectionBooking>): Promise<InspectionBooking> {
  ...
  try {
    ...
    return {
      ...
      studentEmail: bookingData.studentEmail || 'student@dormiqa.africa',
